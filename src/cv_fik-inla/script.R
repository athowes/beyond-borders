...
# orderly::orderly_develop_start("cv_fik-inla")
# setwd("src/cv_fik-inla")
...
#' Function to run bsae::fik_inla for each cross-validation fold
run_cv_models <- function(survey, type) {
  ...
  training_sets <- lapply(training_sets, function(training_set) {
    training_set$fit <- bsae::fik_inla(training_set$data)
    pb$tick()$print()
    return(training_set)
  })
...
#' Function to run bsae::fik_inla for the whole dataset
