...
# orderly::orderly_develop_start("cv_ck-stan")
# setwd("src/cv_ck-stan")
...
#' Function to run bsae::ck_stan for each cross-validation fold
run_cv_models <- function(survey, type) {
  ...
  training_sets <- lapply(training_sets, function(training_set) {
    training_set$fit <- bsae::ck_stan(training_set$data)
    pb$tick()$print()
    return(training_set)
  })
...
#' Function to run bsae::ck_stan for the whole dataset
