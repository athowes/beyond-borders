...
#' Function to run bsae::ik_stan for each cross-validation fold
run_cv_models <- function(survey, type) {
  ...
  training_sets <- lapply(training_sets, function(training_set) {
    training_set$fit <- bsae::ik_stan(training_set$data)
    pb$tick()$print()
    return(training_set)
  })
...
#' Function to run bsae::ik_stan for the whole dataset
