...
# orderly::orderly_develop_start("cv_fck-inla")
# setwd("src/cv_fck-inla")
...
run_cv_models <- function(survey, type) {
  ...
  training_sets <- lapply(training_sets, function(training_set) {
    training_set$fit <- bsae::fck_inla(training_set$data)
    pb$tick()$print()
    return(training_set)
  })
...
#' Function to run bsae::fck_inla for the whole dataset
