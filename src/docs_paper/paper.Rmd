...
Here, we set out to test this hypothesis, and in doing so provide practical recommendations for modelling spatial structure.
...
Finally, we discuss our conclusions and directions for future research in Section \ref{sec:conclusion}.
...
Usually, areal may be conceptualised as arising from aggregation of point data so that $y_i = \int_{A_i} y(s) \text{d}s$.
Exceptions include policies determined at an administrative level, such as the minimum wage or disease control measures, which have a genuinely discrete spatial structure\footnote{For data of this sort, there is less clear justification for using spatial smoothing. Realistically, data relate to many covariates, some number of which are likely have discrete spatial structure and others of which continuous spatial structure, such that the result is often an intermediate. It may be possible to model this explicitly, though the result might look similar to what is achieved using e.g. BYM2.}.
...
Spatial structure is modelled at the level of the latent field, and the observation model, often either binomial or Poisson, is conditionally independent and identically distributed $p(y \, | \, x, \theta) = \prod_{i = 1}^n p(y_i \, | \, x_i, \theta)$.
...
The mean may be modelled using a linear predictor $\mu_i = \beta_0 + z_i^\top \beta$, where $(\beta_0, \beta)$ are regression parameters corresponding to areal covariates^[To denote length $p$ vectors of areal and point covariates, we use $z_i = (z_{i,1}, \ldots, z_{i,p})^\top$ and $z(s) = (z_1(s), \ldots, z_p(s))^\top$ respectively.} $(1, \z_i)^\top$].
Here, we focus on the spatial random effects $\phi$, whose role is to capture the spatial structure between areas not already accounted for in the mean.
...
Specifying $\phi$ amounts to specifying the entries of a precision or covariance matrix, as we discuss below.
...
\begin{equation}
    \phi \sim \mathcal{N}(\mathbf{0}, \tau_\phi^{-1} R^{-}), \label{eq:gmrf}
\end{equation}
where $R^{-}$ is the generalised inverse of the rank-deficient structure matrix\footnote{So-called because it defines the \textit{structure} of the precision matrix $\mathbf{Q} = \tau_\phi R$.} $R$ with entries
\begin{equation}
    R_{ij} =
    \begin{cases}
        n_{\delta i}, & i = j \\
...
\end{equation}
This density is a function of the pairwise differences $\phi_i - \phi_j$ and so is invariant to the addition of a constant $c$ to each entry $p(\phi) = p(\phi + c\mathbf{1})$, leading to an improper uniform distribution on the average of the $\phi_i$.
If $\mathcal{G}$ is connected\footnote{In that by traversing the edges, any vertex can be reached from any other vertex.} then there is only one impropriety in the model and $\text{rank}(R) = n - 1$, while if $\mathcal{G}$ is disconnected, and composed of $n_c \geq 2$ connected components with index sets $I_1, \ldots, I_{n_c}$, then the corresponding structure matrix $R$ has rank $n - n_c$ and the density is invariant to the addition of a constant to each of the connected components $p(\phi_{I}) = p(\phi_{I} + c \mathbf{1})$ where $I = I_1, \ldots, I_{n_c}$.
...
<!-- These models can be specified in terms of scaled weights $\{b_{ij}\}_{j \sim i}$ and a precision vector $\bm{\kappa} = (\kappa_1, \ldots, \kappa_n)^\top$ such that $\phi_i \, | \, \phi_{-i} \sim \N ( \sum_{j: j \sim i} b_{ij} \phi_j, (\kappa_i \tau_\phi)^{-1} )$. -->
<!-- The structure matrix $R$ corresponding to the above full conditionals is given by $R = \mathbf{D}_\kappa(\mathbf{I} - \mathbf{B})$, where $\mathbf{B}$ has the entries $b_{ij}$ for $i \sim j$, diagonal entries $b_{ii} = 0$ and $b_{ij} = 0$ for $i \nsim j$ and the matrix $\mathbf{D}_\kappa$ is given by $\text{diag}(\kappa_1, \ldots, \kappa_n)$. -->
<!-- As the structure matrix is symmetric we must have the condition $-b_{ij}\kappa_i = -b_{ji}\kappa_j$. -->
<!-- To meet this condition, it is often simpler to use an unscaled weights matrix $\mathbf{W} = \mathbf{D}_\kappa \mathbf{B}$ such that $R = \mathbf{D}_\kappa - \mathbf{W}$. -->
...
<!-- The Besag-York-Molli\'{e} (BYM) model of @besag1991bayesian, accounts for this in a natural way by decomposing the spatial random effect $\phi = \bv + \bu$ into a sum of an unstructured IID component $\bv$ and a spatially structured Besag component $\bu$, each of which with their own respective precision parameters $\tau_v$ and $\tau_u$. -->
<!-- The resulting distribution is -->
<!-- \begin{equation} -->
<!--     \phi \sim \N(\mathbf{0}, \tau_v^{-1} \mathbf{I} + \tau_u^{-1} R^{-}) \label{eq:bym}. -->
<!-- \end{equation} -->
...
<!-- \begin{align} -->
<!-- \phi = \frac{1}{\tau_\phi} \left( \sqrt{1- \pi} \, \bv + \sqrt{\pi} \, \bu^\star \right), \label{eq:bym2} -->
<!-- \end{align} -->
<!-- where $\tau_\phi$ is the marginal precision of $\phi$, $\pi \in [0, 1]$ gives the proportion of the marginal variance explained by each component, and $\bu^\star$ is a scaled version of $\bu$ with precision matrix given by the scaled structure matrix $R^\star$ (see~\ref{app:freni}). -->
...
# Areal kernel spatial random effects \label{sec:ks}
...
Another approach is to define the covariance matrix using an areal kernel function $K: \mathcal{P}(\Sc) \times \mathcal{P}(\Sc) \to \mathbb{R}$, where $\mathcal{P}$ denotes the power set such that $\mathcal{P}(\Sc)$ is the space of subsets of the study region, which gives a measure of similarity between two areas.
If $K$ is positive semi-definite, then we define areal kernel spatial random effects by
\begin{equation}
    \phi \sim \mathcal{N}(\mathbf{0}, \frac{1}{\tau_\phi} K), \label{eq:arealkernel}
\end{equation}
...
As such, in the multiple instance learning literature $K$ would be described as a type of kernels on sets [@gartner2002multi].
...
<!-- Various authors [@law2018variational; @tanaka2019spatially; @yousefi2019multi; @hamelijnck2019multi] in the machine learning literature have recently contributed to the development of these aggregated Gaussian process models. -->
...
```{r simulation-geometries, fig.align="center", fig.cap="Simulation geometries."}
knitr::include_graphics("depends/simulation-geometries.pdf")
```
...
We used seven different geometries: the four vignette geometries in Figure \ref{fig:tikz1}, which share an adjacency graph, as well as three more realistic geometries, a $6 \times 6$ lattice grid, the 33 districts of C\^{o}te d'Ivoire and the 36 congressional districts of Texas (Figure \ref{fig:51}).
...
We fit eight inferential models, each of which differing in the spatial random effect specification (Table \ref{tab:inferentialmodels}), allowing us to test sensitivity to spatial structure misspecification.
...
$\phi = \tau_\phi^{-1} \left( \sqrt{1- \pi} \, \bv + \sqrt{\pi} \, \bu^\star \right)$ with a $\text{Beta}(0.5, 0.5)$ prior on $\pi \in [0, 1]$ \\
...
We compared model performance as, again varying the spatial random effect specification, in estimating HIV prevalence in adults aged $15-49$ across four countries in sub-Saharan Africa: Côte d'Ivoire ($n = 33$), Malawi ($n = 28$), Tanzania ($n = 159$) and Zimbabwe ($n = 60$).
...
<!-- Epidemic estimates are required to help plan, implement, and evaluate the success of programmes, as well as allocate limited resources [@hallett2016evaluation; @cuadros2017mapping]. -->
<!-- Household surveys provide nationally representative data about the general population, but are expensive to carry out and as such have small sample sizes at a district level. -->
<!-- Sometimes auxiliary covariates can be used to aid estimation, but data on the covariates most strongly associated with HIV, such as sexual risk behaviour or the prevalence of male circumcision, faces similar measurement difficulties as the HIV indicators themselves. -->
<!-- In light of this fact, models including covariates have only been found to modestly improve predictions \citep[Supplementary Figure 20]{dwyer2019mapping}, unlike other infectious diseases like Malaria where transmission is driven by more predictive and easily-measurable environmental factors [@weiss2015re]. -->
<!-- These circumstances foreground the sharing information between districts via spatial smoothing in HIV mapping. -->

## Household survey data

We used data from the most recent publicly available Population Health Impact Assessment (PHIA) survey in each country.
These surveys utilise a complex design, where each individual $k$ in area $i$ has an unequal probability $\pi_{ik}$ of being included in the sample.
...
To account for the survey design we used sampling weights $\omega_{ik} = 1/\pi_{ik}$ to adjust the raw data in each district, obtaining the Kish effective sample size $m_i^\star = (\sum_k \omega_{ik})^2 / \sum_k \omega_{ik}^2$ and effective number of cases $y_i^\star$.
These effective values may be thought of as what would have been observed had the survey been a simple random sample.

## Model structure

We used a similar model structure as Section \ref{sec:simulation}, with a minor alteration to the likelihood.
The effective number of cases and effective sample size may not be integers, so we modelled the effective number of cases $y_i^\star \in \mathbb{R}$ according to a generalised binomial distribution $y_i^\star \sim \text{xBin}(m_i^\star, \rho_i)$, where $m_i^\star \in \mathbb{R}$ is the effective sample size and $\rho_i \in [0, 1]$ is the adult (15-49) HIV prevalence.
...
\end{equation}
As with the simulation study we linked the prevalence $\rho_i \in [0, 1]$ to $x_i$ via
\begin{equation}
\log \left( \frac{\rho_i}{1 - \rho_i} \right) = x_i = \beta_0 + \phi_i, \label{eq:linear}
\end{equation}
and again considered the range of models for $\bphi$ given by Table \ref{tab:inferentialmodels}.
...
<!-- 	\caption{The $i$th fold in a SLOO-CV in which the model is fitted on the grey (\lightgreyline) areas $A_{-(i, \delta i)}$ with the blue (\lightblueline) areas $A_{\delta i}$ held-out, to be assessed in predicting the green (\lightgreenline) area $A_i$.} -->
<!-- 	\label{fig:sloo} -->
<!-- \end{figure} -->

```{r sloo-cv, fig.align="center", fig.cap="SLOO-CV."}
knitr::include_graphics("depends/sloo-cv.pdf")
```

We assessed each model using two cross-validation approaches: (1) LOO-CV, a standard leave-one-out cross-validation, in which the prevalence data from a single district were removed, and (2) SLOO-CV, a spatial leave-one-out cross-validation, in which the data from a single district and each of its neighbouring districts were removed.
We performed cross-validation manually as we found approximations that avoid refitting the model, such as the built-in conditional predictive ordinate statistic in `R-INLA` package [@held2010posterior] and the Pareto smoothed importance sampling [@vehtari2015pareto] approach implemented in the `loo` package [@vehtari2017practical], to be inaccurate in this setting, perhaps because individual observations in spatial random effect models can exert high influence toward local parameters.

\subsubsection{LOO-CV}
...
To compute the value of CRPS for the $i$th fold we use samples $\{ y_i^s \}_{s = 1}^S$ from the LOO posterior predictive distribution $p(y_i \, | \, y_{-i})$ giving
\begin{equation}
\text{LOO-CRPS}_i = \frac{1}{S} \sum_{s = 1}^S | y_i^s - y_i | - \frac{1}{2S^2} \sum_{s = 1}^S \sum_{l = 1}^S | y_i^s - y_i |. \label{eq:crpsapprox2}
\end{equation}

\subsubsection{SLOO-CV}

If structural dependencies, like spatial dependence, exist in the data then results from LOO-CV tend to underestimate predictive error [@roberts2017cross].
To counteract this effect, we modify the spatial leave-one-out (SLOO) approach of @le2014spatial.
As illustrated by Figure \ref{fig:sloo}, during the $i$th fold of SLOO-CV we leave-out the block $(i, \delta i)$.
This increases the extent to which the training and validation sets are conditionally independent.
The result is a SLOO posterior predictive distribution $p(y_i \, | \, y_{-(i, \delta i)})$ from which, analogously to Equation \ref{eq:crpsapprox2}, we may compute the corresponding value of $\text{SLOO-CRPS}_i$.
