...
#' Function to run bsae::bym2_inla for each cross-validation fold
run_cv_models <- function(survey, type) {
  ...
  training_sets <- lapply(training_sets, function(training_set) {
    training_set$fit <- bsae::bym2_inla(training_set$data)
    pb$tick()$print()
    return(training_set)
  })
...
#' Function to run bsae::bym2_inla for the whole dataset
